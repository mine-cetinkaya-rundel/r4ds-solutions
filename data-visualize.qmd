...
1. Manufacturer,class,fl,drv,model,and trans are all categorical variables.Displ,year,cyl,cty,and hwy are all numerical variables. You can run `glimpse` `mpg` or run ?mpg to see the variables.
...
```{r}
ggplot(mpg,aes(x = hwy, y = displ, size = cty, color = cty, shape = drv)) + 
  geom_point()
```
