...
To facet your plot by a single variable, use [`facet_wrap()`](https://ggplot2.tidyverse.org/reference/facet_wrap.html).
The first argument of [`facet_wrap()`](https://ggplot2.tidyverse.org/reference/facet_wrap.html) is a formula^[2](https://r4ds.hadley.nz/data-visualize.html#fn2)^, which you create with `~` followed by a variable name.
The variable that you pass to [`facet_wrap()`](https://ggplot2.tidyverse.org/reference/facet_wrap.html) should be categorical.
...
Nothing changes in the plot since `linewidth` is not an applicable aesthetic for the point geom.
...
the na.rm argument takes away the warning message.
...
6 categorical,displ is continous,glimpse(mpg) or type mpg or the help function ?m
pg
